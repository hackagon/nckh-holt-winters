---
title: 3.2.2.2.Xac dinh thoi diem tang giam su dung trong mot nam cua cac hoat chat
  su dung nhieu nam 2018
output:
  md_document:
...
bieudo_coso_phanloai <- function(pl){
  ...
  df %>%
    ggplot(aes(x = date, y = coso)) + 
    geom_line() + 
    facet_grid(hoatchat ~ ., scales = "free_y") + 
    theme(strip.background = element_blank(),  axis.title.y = element_text(angle=180, color='red')) +
    geom_point() +
    xlab("Thời gian") + 
    scale_x_date(date_breaks = "1 year", labels = date_format("%Y")) +
    ylab("Cơ số sử dụng") + 
...
bieudo_coso_phanloai_2 <- function(pl){
  vt_hoatchat <- phanloai$hoatchat_dvt[which(phanloai$phanloai == pl)]
  df <- data.frame()
  i <- 1
  while(i <= length(vt_hoatchat)){
  ...
  xyplot(coso ~ date | hoatchat, data = df, type="b",pch=8,
         scales=list(y=list(relation="free")),
       layout=c(1,length(unique(df$hoatchat))),
       xlab = "Năm", ylab = "Cơ số")
}
```
